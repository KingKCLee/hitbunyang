// 분양라인 - 추가 페이지들 (지역현장, 맞춤현장, 관심현장, 서포터즈, FAQ)
  ...
  container.innerHTML = `
    <div style="background:linear-gradient(135deg,#1e3a8a,#2563eb);padding:1.5rem 0;color:white;margin-bottom:1.5rem">
      <div class="container">
        ...
        <p style="opacity:0.8;font-size:0.88rem">지역별 분양 현장을 찾아보세요</p>
      </div>
    </div>
...
function renderRegionSelector() {
  const regions = [
    { name: '서울', icon: '🏙️', color: '#1d4ed8' },
    { name: '경기', icon: '🌆', color: '#065f46' },
    { name: '인천', icon: '✈️', color: '#0f766e' },
    { name: '부산', icon: '🌊', color: '#c2410c' },
    { name: '대구', icon: '🏛️', color: '#b45309' },
    { name: '광주', icon: '🌸', color: '#166534' },
    { name: '대전', icon: '🚄', color: '#6b21a8' },
    { name: '울산', icon: '🏭', color: '#1e3a8a' },
    { name: '세종', icon: '🏛️', color: '#0369a1' },
    { name: '충청', icon: '🌾', color: '#92400e' },
    { name: '전라', icon: '🌊', color: '#065f46' },
    { name: '경상', icon: '⛰️', color: '#991b1b' },
    { name: '강원', icon: '🏔️', color: '#3730a3' },
    ...
    <div class="region-grid" id="region-grid">
      ${regions.map(r => `
        <div class="region-card" onclick="navigate('/region?r=${encodeURIComponent(r.name)}')"
          ...
          <div class="region-card-count" id="region-cnt-${r.name}">로딩중...</div>
        </div>
      `).join('')}
    </div>
  </div>`;
...
async function loadRegionStats() {
  ...
  if (regions) {
    regions.forEach(row => {
      const el = document.getElementById(`region-cnt-${row.region}`);
      if (el) el.textContent = `${(row.property_count || 0).toLocaleString()}건`;
    });
  }
  
  // Also load actual count from DB
  const propsR = await api.get('/properties?limit=1');
  if (propsR.ok) {
    const total = propsR.data.total || 0;
    const regionEls = document.querySelectorAll('[id^="region-cnt-"]');
    if (regionEls.length && total > 0) {
...
async function loadRegionProperties(region, container) {
    ...
    <div style="background:linear-gradient(135deg,#1e3a8a,#2563eb);padding:1.5rem 0;color:white;margin-bottom:1.5rem">
      <div class="container">
        <div style="display:flex;align-items:center;gap:0.75rem">
          ...
          <div>
            <h1 style="font-size:1.5rem;font-weight:900">${escapeHtml(region)} 분양현장</h1>
            <p style="opacity:0.8;font-size:0.88rem">${escapeHtml(region)} 지역의 분양 현장 목록</p>
          </div>
        </div>
      </div>
    </div>
  ...
  const r = await api.get(`/properties?region=${encodeURIComponent(region)}&limit=20`);
  ...
  if (!r.ok || !r.data.data?.length) {
    contentEl.innerHTML = `
      <div class="empty-state">
        ...
        <div>${escapeHtml(region)} 지역의 현장이 없습니다.</div>
  ...
  contentEl.innerHTML = `
    <div style="margin-bottom:1rem;display:flex;justify-content:space-between;align-items:center">
      <div style="font-size:0.88rem;color:#6b7280">총 <strong style="color:#1e40af">${total.toLocaleString()}</strong>건</div>
    </div>
  ...
  container.innerHTML = `
    <div style="background:linear-gradient(135deg,#7c3aed,#a855f7);padding:1.5rem 0;color:white;margin-bottom:1.5rem">
      <div class="container">
        ...
        <p style="opacity:0.8;font-size:0.88rem">내 조건에 맞는 분양 현장을 찾아보세요</p>
      </div>
    </div>
    <div class="container" style="padding-bottom:2rem">
      <!-- Custom filter form -->
      <div style="background:white;border-radius:16px;padding:1.5rem;box-shadow:0 2px 16px rgba(0,0,0,0.08);
        ...
        <h2 style="font-size:1rem;font-weight:800;margin-bottom:1.25rem;color:#1e3a8a">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:1rem">
          <div>
            ...
            <select class="form-select" id="custom-region">
              ...
              ${['서울','경기','인천','부산','대구','광주','대전','충청','전라','경상','강원','제주'].map(r =>
        ...
        <div style="margin-top:1.25rem;display:flex;gap:0.75rem;flex-wrap:wrap">
          <button class="btn btn-primary" onclick="applyCustomFilter()">
            <i class="fas fa-search"></i> 맞춤 현장 검색
          </button>
...
async function applyCustomFilter() {
  ...
  resultsEl.innerHTML = `
    <div style="margin-bottom:1rem;font-size:0.88rem;color:#6b7280">
      총 <strong style="color:#1e40af">${total.toLocaleString()}</strong>건의 현장을 찾았습니다
    </div>
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(270px,1fr));gap:1.25rem">
      ${properties.map(p => renderPropertyCard(p)).join('')}
    </div>`;
...
async function renderFavoritesPage(container) {
  container.innerHTML = `
    <div style="background:linear-gradient(135deg,#db2777,#ec4899);padding:1.5rem 0;color:white;margin-bottom:1.5rem">
      <div class="container">
        <h1 style="font-size:1.5rem;font-weight:900;margin-bottom:0.25rem">
          <i class="fas fa-heart"></i> 관심현장
        </h1>
        <p style="opacity:0.8;font-size:0.88rem">저장한 관심 현장 목록입니다</p>
      </div>
    </div>
    <div class="container" style="padding-bottom:2rem">
  ...
  if (!state.user) {
    document.getElementById('favorites-content').innerHTML = `
      <div class="empty-state">
        ...
        <p style="font-size:0.88rem;margin-bottom:1rem">관심 현장을 저장하려면 로그인해주세요</p>
  ...
  if (!bookmarks.length) {
    content.innerHTML = `
      <div class="empty-state">
        ...
        <div style="font-size:1rem;font-weight:700;margin-bottom:0.5rem">관심 현장이 없습니다</div>
        <p style="font-size:0.88rem;margin-bottom:1rem">마음에 드는 현장의 ♡ 버튼을 눌러 저장하세요</p>
        <button class="btn btn-primary" onclick="navigate('/properties')">현장 둘러보기</button>
      </div>`;
    return;
  }
  ...
  if (r.ok && r.data.data) {
    ...
    if (!favProps.length) {
      content.innerHTML = `<div class="empty-state"><div class="empty-state-icon">❤️</div><div>관심 현장이 없습니다</div></div>`;
      return;
    }
    content.innerHTML = `
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(270px,1fr));gap:1.25rem">
...
async function renderSupportersPage(container) {
  container.innerHTML = `
    <div style="background:linear-gradient(135deg,#065f46,#10b981);padding:1.5rem 0;color:white;margin-bottom:1.5rem">
      <div class="container">
        <h1 style="font-size:1.5rem;font-weight:900;margin-bottom:0.25rem">
          <i class="fas fa-users"></i> 서포터즈
        </h1>
        <p style="opacity:0.8;font-size:0.88rem">분양라인 서포터즈 프로그램에 참여하세요</p>
      </div>
    </div>
    <div class="container" style="padding-bottom:3rem">
      <!-- Hero banner -->
      <div style="background:linear-gradient(135deg,#064e3b,#065f46);border-radius:16px;padding:2.5rem;
        color:white;text-align:center;margin-bottom:2rem;box-shadow:0 4px 24px rgba(6,95,70,0.3)">
        <div style="font-size:3rem;margin-bottom:1rem">🌟</div>
        <h2 style="font-size:1.5rem;font-weight:900;margin-bottom:0.5rem">분양라인 서포터즈가 되어주세요!</h2>
        <p style="opacity:0.85;font-size:0.93rem;margin-bottom:1.5rem;max-width:500px;margin-left:auto;margin-right:auto">
          ...
          서포터즈는 현장 정보 공유, 리뷰 작성, 커뮤니티 활동에 참여합니다.
        </p>
        <div style="display:flex;justify-content:center;gap:1rem;flex-wrap:wrap">
          <button class="btn" style="background:white;color:#065f46;font-weight:700" onclick="navigate('/register')">
      ...
      <div style="margin-bottom:2rem">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1rem">
          ${[
            { icon: '🎟️', title: '포인트 적립', desc: '현장 리뷰 작성, 공유 시 포인트 적립', color: '#fef3c7' },
            { icon: '🎯', title: '우선 정보 제공', desc: '신규 분양 현장 정보 가장 먼저 받기', color: '#dbeafe' },
      ...
      <div style="margin-bottom:2rem">
        ...
        <div style="background:white;border-radius:14px;border:1px solid #e5e7eb;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.07)">
          ${[
            { num: '01', title: '현장 리뷰 작성', desc: '방문한 분양 현장에 대한 리뷰를 작성합니다. 포토리뷰 시 추가 포인트 지급', points: '+50P' },
            { num: '02', title: '분양 정보 공유', desc: 'SNS, 카카오톡 등에 현장 정보 공유 시 포인트 적립', points: '+20P' },
            ...
            <div style="padding:1.25rem 1.5rem;border-bottom:1px solid #f3f4f6;display:flex;align-items:center;gap:1.25rem">
              <div style="background:#1e40af;color:white;font-size:0.75rem;font-weight:800;padding:0.35rem 0.6rem;
              ...
              <div style="background:#d1fae5;color:#065f46;font-weight:800;font-size:0.83rem;padding:0.3rem 0.75rem;border-radius:20px">
                ${a.points}
              </div>
            </div>
          `).join('')}
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:1rem">
          ${['김서포터','이든든님','박활동가','최베스트','정우수','한도우미'].map((name, i) => `
            <div class="supporter-card" style="text-align:center">
              <div style="width:60px;height:60px;border-radius:50%;background:${['#dbeafe','#d1fae5','#fef3c7','#ede9fe','#fce7f3','#ffedd5'][i]};
              ...
              <div style="font-size:0.78rem;color:#1e40af;font-weight:600;margin-top:0.25rem">
  ...
  const faqCategories = [
    {
      ...
      items: [
        { q: '분양라인은 어떤 서비스인가요?', a: '분양라인은 전국 아파트, 오피스텔, 상가 등 분양 현장 정보를 한눈에 확인할 수 있는 부동산 분양 정보 플랫폼입니다. 분양 현장 검색, 구인게시판, 뉴스 등 다양한 서비스를 제공합니다.' },
        { q: '서비스 이용 요금이 있나요?', a: '일반 회원의 경우 무료로 이용하실 수 있습니다. 다만 프리미엄 광고 등록, 현장 상단 노출 등 일부 서비스는 유료로 운영됩니다.' },
        { q: '회원가입 없이도 이용할 수 있나요?', a: '현장 목록 조회, 뉴스/공지 확인 등 기본 서비스는 비회원도 이용 가능합니다. 단, 문의 작성, 관심현장 저장, 알림 설정 등은 회원가입이 필요합니다.' },
        { q: '고객센터 운영 시간은 어떻게 되나요?', a: '고객센터는 평일 09:00 ~ 18:00 운영합니다. 점심시간(12:00 ~ 13:00) 및 주말/공휴일은 운영하지 않습니다. 긴급 문의는 이메일(info@bunyang.com)로 접수해주세요.' },
        { q: '모바일에서도 이용 가능한가요?', a: '네, 분양라인은 모바일 반응형 웹사이트로 스마트폰에서도 편리하게 이용하실 수 있습니다. PC와 동일한 서비스를 모바일 최적화된 화면으로 제공합니다.' },
    ...
    },
    {
      ...
      items: [
        ...
        { q: '비밀번호를 잊어버렸어요.', a: '로그인 페이지에서 "비밀번호 찾기"를 클릭하고, 가입 시 등록한 이메일을 입력하면 임시 비밀번호가 발송됩니다. 고객센터(1660-0464)로 문의하셔도 됩니다.' },
        { q: '회원 탈퇴는 어떻게 하나요?', a: '마이페이지 → 계정 설정 → 회원 탈퇴에서 탈퇴 처리가 가능합니다. 탈퇴 후 30일 동안은 개인정보가 보관되며, 이후 영구 삭제됩니다.' },
        { q: '기업 회원과 개인 회원의 차이는?', a: '기업 회원은 분양 현장 등록, 구인 공고 작성, 기업 인증 뱃지 등 추가 기능을 이용할 수 있습니다. 기업명, 사업자등록번호 등을 추가로 입력해야 합니다.' },
        { q: '이메일 변경이 가능한가요?', a: '현재 이메일 변경은 고객센터를 통해서만 가능합니다. 본인 확인 후 변경 처리해 드립니다. 고객센터: 1660-0464' },
    ...
    {
      id: 'property', label: '분양 현장', icon: '🏢',
      items: [
        { q: '분양 현장은 어떻게 등록하나요?', a: '회원 로그인 후 상단의 "현장 등록" 버튼을 클릭하거나 분양현장 목록에서 등록할 수 있습니다. 현장명, 위치, 가격 등 정보를 입력하면 됩니다.' },
        { q: '등록된 현장 정보는 언제 노출되나요?', a: '일반 등록 현장은 검토 후 즉시 노출됩니다. 단, 허위 정보나 규정 위반 시 승인이 거부될 수 있습니다.' },
        { q: '현장 정보 수정은 가능한가요?', a: '등록자 본인 또는 관리자만 현장 정보를 수정할 수 있습니다. 마이페이지 또는 현장 상세 페이지에서 수정하세요.' },
        { q: '베스트 현장은 어떻게 선정되나요?', a: '조회수, 문의수, 공유수, 검색 노출수 등을 종합한 점수로 순위가 결정됩니다. 조회수 1점 + 문의수 3점 + 공유수 2점으로 계산됩니다.' },
        { q: '현장 이미지는 어떻게 등록하나요?', a: '현장 등록/수정 시 이미지 업로드 기능을 통해 등록 가능합니다. 최대 10장까지 등록 가능하며, JPG/PNG 형식을 지원합니다.' },
        { q: '분양 완료된 현장도 볼 수 있나요?', a: '네, 분양 완료된 현장도 조회 가능합니다. 필터에서 "분양완료"를 선택하면 됩니다.' },
        { q: '현장 문의는 어떻게 하나요?', a: '현장 상세 페이지 하단의 "문의 보내기" 양식을 이용하거나, 등록된 전화번호로 직접 연락하실 수 있습니다.' },
      ]
    },
    {
      id: 'jobs', label: '구인게시판', icon: '💼',
      items: [
        { q: '구인 공고는 어떻게 등록하나요?', a: '기업 회원 로그인 후 구인게시판에서 "공고 등록" 버튼을 클릭하여 등록할 수 있습니다. 현장명, 지역, 직위, 급여 조건 등을 상세히 입력해주세요.' },
        { q: '팀장과 팀원의 차이는 무엇인가요?', a: '팀장은 팀을 이끌며 관리 업무도 함께 수행하는 역할이고, 팀원은 분양 현장에서 고객 안내 및 계약 업무를 담당합니다. 수수료율 등 조건이 다를 수 있습니다.' },
        { q: '수수료(커미션)는 어떻게 지급되나요?', a: '수수료율은 현장과 계약에 따라 다릅니다. 일반적으로 계약 체결 후 일정 기간 내 지급되며, 공고에 명시된 조건을 확인해주세요.' },
        { q: '일당이 있는 현장은 어디서 확인하나요?', a: '구인게시판 필터에서 "일당 있음"을 선택하면 일당을 지급하는 현장만 필터링됩니다.' },
        { q: '숙소 제공 여부는 어떻게 확인하나요?', a: '구인 공고 ��세 페이지에서 숙소 제공 여부를 확인할 수 있습니다. 필터에서 "숙소 있음"을 선택하면 숙소를 제공하는 현장만 볼 수 있습니다.' },
        { q: '급구 공고가 무엇인가요?', a: '즉시 업무 시작이 가능한 현장으로, 채용이 시급한 경우 "급구" 뱃지가 표시됩니다. 빠른 합류가 가능한 경우 지원을 고려해보세요.' },
        { q: '경력자만 지원 가능한가요?', a: '현장별로 다릅니다. 공고에 경력 조건이 명시되어 있으며, "신입 가능" 조건의 현장도 있습니다. 상세 조건은 공고를 확인해주세요.' },
      ]
    },
    {
      ...
      items: [
        ...
        { q: '광고 신청 방법을 알고 싶어요.', a: '고객센터(1660-0464)로 연락하시거나 이메일(info@bunyang.com)로 광고 문의해주세요. 담당자가 안내드립니다.' },
        ...
        { q: '프리미엄 현장은 어떻게 노출되나요?', a: '프리미엄 현장은 목록 최상단에 고정 노출되며, 황금색 테두리로 구분됩니다. 검색 결과에서도 우선 노출됩니다.' },
        { q: '광고 효과는 어떻게 확인할 수 있나요?', a: '광고주 전용 대시보드에서 클릭수, 노출수, 문의수 등 광고 성과를 실시간으로 확인하실 수 있습니다.' },
    ...
    },
    {
      ...
      items: [
        { q: '지도 서비스는 어떻게 이용하나요?', a: '"지도현장" 메뉴에서 전국 분양 현장을 지도에서 확인할 수 있습니다. 내 위치 기반 검색과 주소 검색을 지원합니다.' },
        { q: '알림 설정은 어떻게 하나요?', a: '맞춤현장 메뉴에서 원하는 조건을 설정하고 "알림 설정" 버튼을 클릭하면 됩니다. 새 현장 등록 시 이메일 또는 앱 알림을 받을 수 있습니다.' },
        { q: '현장 공유는 어떻게 하나요?', a: '현장 상세 페이지에서 공유 버튼을 클릭하면 카카오톡, 링크 복사 등으로 공유할 수 있습니다.' },
        { q: '검색이 잘 안 될 때는?', a: '지역명과 현장명으로 검색 가능합니다. 띄어쓰기를 포함하거나 제외하고 검색해보세요. 여전히 문제가 있다면 고객센터로 문의해주세요.' },
        { q: '즐겨찾기(관심현장)는 몇 개까지 등록되나요?', a: '현재 제한 없이 등록 가능합니다. 로그인 상태에서 현장 상세 페이지의 하트(♥) 버튼을 누르면 추가됩니다.' },
        { q: '인터넷 익스플로러에서도 되나요?', a: '인터넷 익스플로러는 지원하지 않습니다. Chrome, Safari, Edge, Firefox 등 최신 브라우저 사용을 권장합니다.' },
        { q: '오류 신고는 어떻게 하나요?', a: '고객센터(1660-0464) 또는 이메일(info@bunyang.com)로 오류 내용, 오류 발생 화면, 기기 및 브라우저 정보를 포함하여 신고해주세요.' },
      ]
    },
    {
      ...
      items: [
        ...
        { q: '세금계산서 발급이 가능한가요?', a: '기업 회원의 경우 광고비 결제 후 세금계산서 발급이 가능합니다. 고객센터(1660-0464)로 요청해주세요.' },
        { q: '결제 오류가 발생했어요.', a: '결제 오류 시 고객센터(1660-0464)로 즉시 연락해주세요. 카드사 승인 내역과 오류 메시지를 알려주시면 신속히 처리해드립니다.' },
      ]
    },
  ];
  
  container.innerHTML = `
    <div style="background:linear-gradient(135deg,#0c4a6e,#0ea5e9);padding:1.5rem 0;color:white;margin-bottom:1.5rem">
      <div class="container">
        <h1 style="font-size:1.5rem;font-weight:900;margin-bottom:0.25rem">
          <i class="fas fa-question-circle"></i> 고객센터 / FAQ
        </h1>
        <p style="opacity:0.8;font-size:0.88rem">자주 묻는 질문과 고객센터 안내입니다</p>
      </div>
    </div>
    <div class="container" style="padding-bottom:3rem">
      <!-- Contact info cards -->
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1rem;margin-bottom:2rem">
        <div style="background:white;border-radius:14px;padding:1.5rem;border:1px solid #e5e7eb;
          ...
          <div style="font-weight:800;font-size:1rem;margin-bottom:0.25rem">전화 상담</div>
          <div style="font-size:1.3rem;font-weight:900;color:#1e40af;margin-bottom:0.25rem">
            <a href="tel:16600464" style="color:inherit;text-decoration:none">1660-0464</a>
          </div>
        ...
        <div style="background:white;border-radius:14px;padding:1.5rem;border:1px solid #e5e7eb;
          ...
          <div style="font-size:1.1rem;font-weight:700;color:#1e40af;margin-bottom:0.25rem">02-000-0000</div>
        ...
        <div style="background:white;border-radius:14px;padding:1.5rem;border:1px solid #e5e7eb;
          ...
          <div style="font-size:0.95rem;font-weight:700;color:#1e40af;margin-bottom:0.25rem">
            <a href="mailto:info@bunyang.com" style="color:inherit;text-decoration:none">info@bunyang.com</a>
          </div>
        ...
        <div style="background:white;border-radius:14px;padding:1.5rem;border:1px solid #e5e7eb;
          ...
          <div style="font-size:0.95rem;font-weight:700;color:#1e40af;margin-bottom:0.25rem">@분양라인</div>
          <div style="font-size:0.78rem;color:#6b7280">평일 09:00 ~ 18:00</div>
        </div>
      </div>
      ...
      <div id="faq-list">
        ${faqCategories.map(cat => `
          <div class="faq-category" data-cat="${cat.id}">
            <div style="font-size:0.9rem;font-weight:700;color:#1e3a8a;margin-bottom:0.75rem;
              padding:0.5rem 1rem;background:#eff6ff;border-radius:8px;display:flex;align-items:center;gap:0.5rem">
              ${cat.icon} ${cat.label}
            </div>
              ...
              <div class="faq-item" data-cat="${cat.id}">
                ...
                <div class="faq-answer">
                  <div style="display:flex;gap:0.75rem;padding:0.75rem 0">
                    <span style="background:#10b981;color:white;font-size:0.75rem;font-weight:800;
      ...
      <div style="background:white;border-radius:16px;border:1px solid #e5e7eb;padding:2rem;
        ...
        <h2 style="font-size:1.1rem;font-weight:800;margin-bottom:1.25rem;color:#1e3a8a">
          <i class="fas fa-envelope"></i> 1:1 문의 보내기
        </h2>
...
async function submitContactForm() {
  ...
  if (r.ok) {
    ...
    alert('문의 접수에 실패했습니다. 고객센터(1660-0464)로 전화 주세요.');
  }
}
